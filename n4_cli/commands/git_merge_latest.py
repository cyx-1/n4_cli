...
def git_merge_latest(dry_run):
    ...
    commands = [
        ...
        ("git for-each-ref --sort=-committerdate refs/remotes/origin --format='%(objectname:short)' | head -1", "Find latest commit across all remote branches"),
    ]
    ...
    latest_commit = run_git_command(
        "git for-each-ref --sort=-committerdate refs/remotes/origin --format='%(objectname:short)' | head -1",
